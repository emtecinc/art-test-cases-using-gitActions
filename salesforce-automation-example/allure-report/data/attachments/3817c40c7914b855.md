...
```
Error: searchFrame not found in Plan Lookup popup
```
          ...
          - list [ref=e36]:
            ...
            - listitem [ref=e39]:
              - link "Tasks" [ref=e40] [cursor=pointer]:
                - /url: /servlet/servlet.Integration?lid=01rG0000000owZn&ic=1&linkToken=VmpFPSxNakF5Tmkwd05DMHhNbFF3T0Rvd09Ub3pOUzR6TVROYSw4eUhBMzdYNjB0c0UtclhlWHFJeVZfWGhhQTB2YjF5dklETVl5dUhsSVdnPSxZV1prTUdKaA%3D%3D
            - listitem [ref=e41]:
          ...
          - cell [ref=e71]:
            ...
            - generic [ref=e82]:
              ...
              - generic [ref=e93]:
                ...
                - table [ref=e101]:
                  - rowgroup [ref=e102]:
                    - row "* Opportunity Name ClaClassic_1775722175119 Opportunity Owner Somya Sijaria" [ref=e103]:
                      - cell "* Opportunity Name" [ref=e104]:
                        - generic [ref=e105]:
                          - generic [ref=e106]: "*"
                          - text: Opportunity Name
                      - cell "ClaClassic_1775722175119" [ref=e107]:
                        - textbox "* Opportunity Name" [ref=e110]: ClaClassic_1775722175119
                    ...
                    - row "* Close Date [ 4/9/2026 ] Actual Destination --None--" [ref=e239]:
                      ...
                      - cell "--None--" [ref=e251]:
                        - combobox "Actual Destination" [ref=e253]:
                          ...
                          - option "Annuity"
                          - option "Other"
                    - row "Actual Close Date [ 4/9/2026, 4:09 AM ]" [ref=e254]:
                      - cell [ref=e255]
                      - cell [ref=e256]
                      - cell "Actual Close Date" [ref=e257]
                      - cell "[ 4/9/2026, 4:09 AM ]" [ref=e258]:
                        - generic [ref=e259]:
                          - textbox "Actual Close Date" [ref=e260]
                          - generic [ref=e261]:
                            - text: "["
                            - link "4/9/2026, 4:09 AM" [ref=e262] [cursor=pointer]:
                              - /url: javascript:void(DatePicker.insertDate('4/9/2026, 4:09 AM', '00NG000000DwsUF', true));
                            - text: "]"
...
```ts
  ...
  120 |    * with two named frames: "searchFrame" (has search input) and "resultsFrame" (has links).
  121 |    * We access them via `popupPage.frame({ name: '...' })`.
  122 |    *
  ...
  130 | 
  131 |       // The popup is a frameset — wait for both frames to load
  132 |       await popupPage.waitForLoadState('domcontentloaded');
  133 | 
  134 |       // Access the "searchFrame" which contains the search input and Go button
  135 |       const searchFrame = popupPage.frame({ name: 'searchFrame' });
> 136 |       if (!searchFrame) throw new Error('searchFrame not found in Plan Lookup popup');
      |                               ^ Error: searchFrame not found in Plan Lookup popup
  137 | 
  138 |       const searchInput = searchFrame.locator('input[name="lksrch"]');
  139 |       await expect(searchInput).toBeVisible({ timeout: 15_000 });
  140 |       await searchInput.fill(planName);
  141 |       await searchFrame.locator('input[name="go"]').click();
  142 | 
  143 |       // Results appear in "resultsFrame"
  144 |       const resultsFrame = popupPage.frame({ name: 'resultsFrame' });
  145 |       if (!resultsFrame) throw new Error('resultsFrame not found in Plan Lookup popup');
  146 | 
  147 |       const resultLink = resultsFrame.getByRole('link', { name: planName });
  148 |       await expect(resultLink).toBeVisible({ timeout: 15_000 });
  149 |       await resultLink.click();
  150 | 
  151 |       // Popup closes after selection — wait for it to close
  152 |       await popupPage.waitForEvent('close', { timeout: 15_000 });
  153 |     } catch (error) {
  154 |       console.error(`Failed to fill Plan lookup and select: ${planName}`);
  155 |       throw error;
  156 |     }
  157 |   }
  158 | 
  159 |   async selectStage(stage: string): Promise<void> {
  160 |     try {
  161 |       await this.stageSelect.getLocator().selectOption(stage);
  162 |     } catch (error) {
  163 |       console.error(`Failed to select Stage: ${stage}`);
  164 |       throw error;
  165 |     }
  166 |   }
  167 | 
  168 |   async fillCloseDate(date: string): Promise<void> {
  169 |     try {
  170 |       const input = this.closeDateInput.getLocator();
  171 |       await input.fill(date);
  172 |       await input.press('Tab');
  173 |     } catch (error) {
  174 |       console.error(`Failed to fill Close Date: ${date}`);
  175 |       throw error;
  176 |     }
  177 |   }
  178 | 
  179 |   /**
  180 |    * Clicks Save and waits for the redirect to the Opportunity detail page.
  181 |    * After saving, Salesforce Classic either stays in Classic or redirects to
  182 |    * the Lightning detail page depending on org settings. We wait for navigation
  183 |    * to complete.
  184 |    */
  185 |   async clickSave(): Promise<void> {
  186 |     try {
  187 |       await this.saveButton.getLocator().click();
  188 |       // Wait for navigation away from the edit form
  189 |       await this['page'].waitForURL((url) => !url.href.includes('/006/e'), { timeout: 30_000 });
  190 |     } catch (error) {
  191 |       console.error('Failed to click Save on Classic Opportunity form');
  192 |       throw error;
  193 |     }
  194 |   }
  195 | 
  196 |   // ── Verification ──────────────────────────────────────────────────────────
  197 | 
  198 |   async verifyFormLoaded(): Promise<void> {
  199 |     try {
  200 |       await expect(this.opportunityNameInput.getLocator()).toBeVisible({ timeout: 20_000 });
  201 |     } catch (error) {
  202 |       console.error('Classic Opportunity creation form is not visible');
  203 |       throw error;
  204 |     }
  205 |   }
  206 | }
