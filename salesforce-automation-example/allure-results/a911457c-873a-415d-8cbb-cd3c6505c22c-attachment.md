...
```ts
  ...
  75 |     try {
  76 |       // Wait for Lightning spinner to clear after page load/navigation
  77 |       await this['page'].locator('.slds-spinner').waitFor({ state: 'hidden', timeout: 30_000 }).catch(() => {});
  78 |       // Salesforce Lightning heading includes the object type prefix
  79 |       // e.g. "Opportunity Rol - - TestEmployer_xxx" — so never use exact: true.
  80 |       const title = new ResilientLocator(this['page'], [
  81 |         (p) => p.getByRole('heading', { name: opportunityName }),
  82 |         (p) => p.locator('h1').filter({ hasText: opportunityName }),
  83 |         (p) => p.locator('.slds-page-header__title').filter({ hasText: opportunityName }),
  84 |       ]);
> 85 |       await expect(title.getLocator()).toBeVisible({ timeout: 30_000 });
     |                                        ^ Error: expect(locator).toBeVisible() failed
  86 |     } catch (error) {
  87 |       console.error(`Failed to verify opportunity detail page for: ${opportunityName}`);
  88 |       throw error;
  89 |     }
  90 |   }
  91 | }
