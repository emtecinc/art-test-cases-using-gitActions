...
export class EmailVerificationService {
  constructor(
    ...
    private readonly inboxProvider: IInboxProvider
  ) {}

  /**
   * Mailosaur verification
   */
  async verifyWithMailosaur(
    query: EmailQuery,
    retryConfig?: Partial<RetryConfig>
  ): Promise<InboxEmail[]> {
    const inboxMatches = await this.inboxProvider.findMatchingEmails(
        query.sentAfter,
        query.subject,
        query.address,
        retryConfig
  ...
  async verify(
    ...
    console.log(
      `[EmailVerificationService] Fetching emails from provider (subject="${query.subject}", recipient="${query.address}")…`
    );
    const sentEmails = await this.emailProvider.fetchEmails(query);

    if (sentEmails.length === 0) {
      console.log(
        '[EmailVerificationService] No emails found in the provider — nothing to verify.'
      );
      return { allVerified: false, matchedEmails: [], unmatchedEmails: [] };
    }

    console.log(
      `[EmailVerificationService] Found ${sentEmails.length} email(s) in the provider.`
    );

    // Step 2 — For each sent email, look for a matching inbox message
    const matched: MatchedEmailPair[] = [];
    const unmatched: EmailRecord[] = [];

    for (const sentEmail of sentEmails) {
      console.log(
        `[EmailVerificationService] Verifying inbox delivery for email id=${sentEmail.id} subject="${sentEmail.subject}"…`
      );
    ...
    console.log(
      `[EmailVerificationService] Verification complete — ` +
