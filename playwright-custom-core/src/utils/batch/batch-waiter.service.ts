...
import { getBatchStatus } from './batch-status.service';
...
export async function waitForBatchCompletion(
  ...
  while (status !== 'Completed' && attempts < maxAttempts) {
    ...
    const job = await getBatchStatus(credentials, batchClassName);
    status = job.Status;

    console.log(
      `[BatchWaiter] Attempt ${attempts}/${maxAttempts} - ` +
      `Status: ${status} - ` +
  ...
  throw new Error(
    `Batch polling timeout after ${attempts} attempts. ` +
    `Last status: ${status}. ` +
