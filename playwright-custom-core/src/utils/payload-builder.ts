 ...
 * ```
 */
export class PayloadBuilder<T extends Record<string, unknown> = Record<string, unknown>> {
  private payload: T;
   ...
   */
  static fromFile<T extends Record<string, unknown> = Record<string, unknown>>(
    filePath: string
  ): PayloadBuilder<T> {
    ...
    try {
      ...
      return new PayloadBuilder<T>(parsed);
    } catch (error) {
      throw new Error(
        `Invalid JSON in template ${resolvedPath}: ${(error as Error).message}`
      );
    }
  }
  ...
  static fromObject<T extends Record<string, unknown> = Record<string, unknown>>(
    ...
    return new PayloadBuilder<T>(base);
  }
  ...
  static empty<T extends Record<string, unknown> = Record<string, unknown>>(): PayloadBuilder<T> {
    return new PayloadBuilder<T>({} as T);
  }
  ...
  set<K extends keyof T>(key: K, value: T[K] | null | undefined): PayloadBuilder<T> {
    ...
    if (this.strictMode && !(key in this.payload)) {
      throw new Error(
        `Strict mode: key "${String(key)}" does not exist in base payload. ` +
  ...
  setNested(keyPath: string, value: unknown): PayloadBuilder<T> {
    ...
    let current: Record<string, unknown> = this.payload as Record<string, unknown>;
    ...
    for (let i = 0; i < keys.length - 1; i++) {
      ...
      if (this.strictMode && !(key in current)) {
        throw new Error(
          `Strict mode: nested key "${key}" (in path "${keyPath}") does not exist in payload`
        );
      }
      ...
      if (current[key] === null || current[key] === undefined || typeof current[key] !== 'object') {
        if (this.strictMode) {
          throw new Error(
            `Strict mode: cannot create intermediate object for "${key}" in path "${keyPath}"`
          );
        }
        current[key] = {};
      }
    ...
    if (this.strictMode && !(finalKey in current)) {
      throw new Error(
        `Strict mode: key "${finalKey}" (in path "${keyPath}") does not exist in payload`
      );
    }
  ...
  merge(overrides: Partial<T> | Record<string, unknown>): PayloadBuilder<T> {
    ...
    for (const [key, value] of Object.entries(overrides)) {
      ...
      if (this.strictMode && !(key in this.payload)) {
        throw new Error(
          `Strict mode: key "${key}" does not exist in base payload. ` +
            `Available keys: ${Object.keys(this.payload).join(', ')}`
        );
      }
