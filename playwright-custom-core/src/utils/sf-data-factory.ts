import {SalesforceConnection} from '../utils/index';
import dotenv from 'dotenv';

dotenv.config();

// ============================================================
// COMPOSITE API — TYPE DEFINITIONS
// ============================================================
...
* ```
*/
export class SFDataFactory {
    private accessToken: string = '';
    private instanceUrl: string = '';
    private recordsToCleanup: { type: string; id: string; name?: string }[] = [];
    ...
    async authenticate(): Promise<void> {
            if (this.accessToken) return; // Already populated — skip
    ...
    async getRecordIdByField(
        ...
        const query = encodeURIComponent(
            `SELECT Id FROM ${sObject} WHERE ${fieldName} = '${value}' LIMIT 1`
        );
        const response = await fetch(
            `${this.instanceUrl}/services/data/v65.0/query?q=${query}`,
            {
    ...
    registerForCleanup(sObject: string, id: string, name?: string): void {
        ...
        const alreadyRegistered = this.recordsToCleanup.some(
            (r) => r.id === id && r.type === sObject
        );
        ...
        console.log(
            `📌 Registered for cleanup: ${sObject} (${id})${name ? ` - ${name}` : ''}`
        );
    }
    ...
    async teardown(): Promise<void> {
        ...
        for (const record of this.recordsToCleanup.reverse()) {
            try {
                const response = await fetch(
                    `${this.instanceUrl}/services/data/v65.0/sobjects/${record.type}/${record.id}`,
                    {
                        method: 'DELETE',
                        headers: { Authorization: `Bearer ${this.accessToken}` },
                    }
                ...
                if (response.ok || response.status === 404) {
                    console.log(
                        `✅ Deleted ${record.type}: ${record.id}${record.name ? ` - ${record.name}` : ''}`
                    );
                    successCount++;
                } else {
                    const error = await response.text();
                    console.error(
                        `❌ Failed to delete ${record.type} ${record.id}: ${error}`
                    );
                    failCount++;
                }
            } catch (error) {
                console.error(
                    `❌ Exception deleting ${record.type} ${record.id}:`,
                    error
                );
                failCount++;
            }
     ...
     * ```
     */
    registerRecordFromUrl(
        url: string,
        name?: string,
    ): { objectApiName: string; recordId: string } {
        const match = url.match(/\/lightning\/r\/([A-Za-z_][A-Za-z0-9_]*)\/([a-zA-Z0-9]{15,18})\/view/);
        if (!match) {
            throw new Error(
                `Cannot extract record info from URL: ${url}. Expected format: /lightning/r/<Object>/<RecordId>/view`,
            );
        }
    ...
    async createRecord(
        ...
        const response = await fetch(
            `${this.instanceUrl}/services/data/v65.0/sobjects/${sObject}`,
            {
                method: 'POST',
                headers: {
        ...
        if (!response.ok) {
            ...
            throw new Error(
                `Failed to create ${sObject}: ${JSON.stringify(errors)}`
            );
        }
    ...
    async updateRecord(
        ...
        const response = await fetch(
            `${this.instanceUrl}/services/data/v65.0/sobjects/${sObject}/${recordId}`,
            {
                method: 'PATCH',
                headers: {
                    Authorization: `Bearer ${this.accessToken}`,
        ...
        if (!response.ok) { 
            ...
            throw new Error(
                `Failed to update ${sObject} (${recordId}): ${JSON.stringify(errors)}`
            );
        }
     ...
     */
    async fetchRecord<T extends Record<string, unknown> = Record<string, unknown>>(
        sObject: string,
        recordId: string,
        fields?: string[]
    ): Promise<T> {
        ...
        if (!response.ok) {
            const errorText = await response.text();
            let errors: unknown;
            try { errors = JSON.parse(errorText); } catch { errors = errorText; }
    ...
    async deleteRecord(
        ...
        if (!response.ok && response.status !== 404) {
            ...
            throw new Error(
                `Failed to delete ${sObject} (${recordId}): ${JSON.stringify(errors)}`
            );
        }
     ...
     * ```
     */
    async executeCompositeRequest(
        subrequests: CompositeSubrequest[],
        allOrNone: boolean = false
    ): Promise<CompositeSubresponse[]> {
        // await this.authenticate();
        ...
        if (subrequests.length > 25) {
            throw new Error(
                `executeCompositeRequest: Salesforce Composite API allows at most 25 subrequests per call, got ${subrequests.length}`
            );
        }
        ...
        const response = await fetch(
            `${this.instanceUrl}/services/data/v65.0/composite/`,
            {
                method: 'POST',
                headers: {
                    Authorization: `Bearer ${this.accessToken}`,
        ...
        if (!response.ok) {
            ...
            try { errors = JSON.parse(errorText); } catch { errors = errorText; }
            throw new Error(
                `Composite API request failed (${response.status}): ${JSON.stringify(errors)}`
            );
        }
        ...
        const failed = result.compositeResponse.filter(
        ...
        if (failed.length > 0) {
            const details = failed
                ...
                .join('\n  ');
            throw new Error(
                `Composite API: ${failed.length} subrequest(s) failed:\n  ${details}`
            );
        }

        console.log(
            `🔗 Composite API: ${subrequests.length} subrequest(s) completed successfully`
        );
     ...
     */
    getRegisteredRecords(): ReadonlyArray<{ type: string; id: string; name?: string }> {
