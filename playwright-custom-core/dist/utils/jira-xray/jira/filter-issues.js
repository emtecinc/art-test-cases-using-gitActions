...
exports.filterIssueKeys = filterIssueKeys;
 ...
 */
async function filterIssueKeys(keys, filter, options = {}) {
    if (keys.length === 0) {
    ...
    const { issues } = await (0, search_issues_1.findIssues)({
        issueKeys: keys,
    ...
    console.log(`  → ${filtered.length} of ${keys.length} key(s) matched:`, filtered.join(', ') || '(none)');
    return filtered;
}
//# sourceMappingURL=filter-issues.js.map