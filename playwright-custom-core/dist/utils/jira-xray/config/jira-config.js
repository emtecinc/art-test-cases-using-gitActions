...
exports.jiraRequestConfig = jiraRequestConfig;
// ── Public API ───────────────────────────────────────────────────
/**
...
function resolveJiraConfig(config) {
    ...
    if (!baseUrl)
        throw new Error('Jira base URL is required (config.baseUrl or JIRA_URL env var).');
    if (!email || !apiToken)
 ...
 */
function jiraRequestConfig(resolved) {
    return {
        auth: resolved.auth,
        headers: {
