...
exports.ImpersonationHelper = void 0;
...
class ImpersonationHelper {
     ...
     */
    static resolveBaseUrl(page, baseUrl) {
        ...
        throw new Error('[ImpersonationHelper] Cannot resolve Salesforce base URL. ' +
     ...
     */
    static async loginAs(page, targetUserDisplayName, baseUrl) {
        ...
        if (!loginLink) {
            throw new Error(`[ImpersonationHelper] Could not find user "${targetUserDisplayName}" ` +
        ...
        console.log(`[ImpersonationHelper] ✅ Now impersonating "${targetUserDisplayName}". ` +
            `URL: ${page.url()}`);
    }
    // ============================================
     ...
     */
    static isOnLoginPage(page) {
        const currentUrl = page.url();
        return ImpersonationHelper.LOGIN_PAGE_SIGNALS.some((signal) => currentUrl.includes(signal));
    }
    // ============================================
     ...
     * @returns true if Lightning shell is reachable after restoring cookies.
     */
    static async restoreViaJwtRefresh(page) {
        const logTag = '[ImpersonationHelper]';
        ...
        try {
            ...
            const { accessToken, instanceUrl } = await salesforce_connection_1.SalesforceConnection.getInstance().getConnection();
     ...
     */
    static async logBack(page, baseUrl) {
        const logTag = '[ImpersonationHelper]';
        ...
        if (!clicked) {
            throw new Error(`${logTag} Could not find "Log out as" link. ` +
        ...
        console.log(`${logTag} ⚠️  Redirected to login page after impersonation logout. ` +
        ...
        throw new Error(`${logTag} ❌ Failed to restore admin session after impersonation logout. ` +
            `All recovery attempts exhausted. Current URL: ${page.url()}`);
    }
}
exports.ImpersonationHelper = ImpersonationHelper;
//# sourceMappingURL=impersonation-helper.js.map