...
class TestDataGenerator {
     ...
     */
    static uniqueEmail(prefix = 'testuser', domain = 'test.example.com') {
     ...
     */
    static resolvePlaceholders(data) {
        const resolved = {};
        const timestamp = Date.now().toString();
        for (const [key, value] of Object.entries(data)) {
            let result = value;
            result = result.replace(/\{\{UNIQUE\}\}/g, timestamp);
            result = result.replace(/\{\{UNIQUE_NAME:(\w+)\}\}/g, (_, prefix) => `${prefix}_${timestamp}`);
            result = result.replace(/\{\{UNIQUE_SERIAL:(\w+)\}\}/g, (_, prefix) => `${prefix}-${timestamp}`);
